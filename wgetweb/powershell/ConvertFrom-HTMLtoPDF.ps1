...
try {
                ...
                $PSScriptRoot1 = "D:\wgetweb\powershell\domino"
                $FilesInFolder = Get-ChildItem  $PSScriptRoot1 -filter "*.html"
                #echo $PSScriptRoot1
                ForEach($File in $FilesInFolder) {
                     #echo $File.FullName
                    # Convert-PDF -HTML $File.FullName
                    $HtmlFile =  $File.FullName 
                    $PdfFile= $HtmlFile -replace '.html$','.pdf' 
                    #$reader = Get-Content $HtmlFile
                    #echo $reader
                    $HTMLCode = Get-Content $HtmlFile 
                    ConvertFrom-HTMLtoPDF -Source  $HTMLCode -Destination $PdfFile -Verbose
                }
